...
def main():
    ...
    num_models = 4
    devices_per_model = [[0,1,2,3], [0,1,2,3]
                        [4,5,6,7], [4,5,6,7]]
    ...
    for _ in range(num_models):
        models.append(
            eval('models.'+cfg.MODEL.NAME+'.get_pose_net')(
                cfg, is_train=True
            )
        )

    semantic_augmentation_pool = []
    for s in part_scale:
        for v in vertical_translate:
            for h in horizontal_translate:
                for n in num_parts_per_augmentation:
                    semantic_augmentation_pool.append(
    ...
    # writer_dict['writer'].add_graph(models[0], (dump_input, ))

    logger.info(get_model_summary(models[0], dump_input))

    for i in range(num_models):
        models[i] = torch.nn.DataParallel(models[i], device_ids=devices_per_model[i]).cuda()
    ...
    size_semantic_augmentation_pool = len(semantic_augmentation_pool)
    theta = nn.Parameter(nn.ones(size_semantic_augmentation_pool) / size_semantic_augmentation_pool, requires_grad=True)
    aug_prob_distribution = torch.nn.functional.softmax(theta)
    semantic_augmentation_idxes = []
    for i in range(num_models):
        semantic_augmentation_idxes.append(torch.torch.multinomial(aug_prob_distribution, len(train_datasets[i]), replacement=True))
        setattr(train_datasets[i], 'semantic_augmentation_pool', semantic_augmentation_pool)
        setattr(train_datasets[i], 'aug_idxes', semantic_augmentation_idxes)
