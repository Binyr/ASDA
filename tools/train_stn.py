...
def main():
    ...
    train_dataset = eval('dataset.'+cfg.DATASET.DATASET)(
        cfg, cfg.DATASET.ROOT, cfg.DATASET.TRAIN_SET, True, True,
        transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ])
    ...
    stn_checkpoint_file = os.path.join(
        final_output_dir, 'stn_checkpoint.pth'
    )
    ...
    # import numpy as np
    # train_cropped = np.zeros((len(train_dataset, 16, 3)))
    # idx = 0
    # for i, (input, target, target_weight, meta) in enumerate(train_loader):
    #     joints = meta['joints'].numpy()
    #     joints_vis = meta['joints_vis'].numpy()
    #     joints = np.concatenate([joints, joints_vis], axis=2)
    #     num_image = input.size(0)
    #     train_cropped[idx:idx+num_image] = joints
    #     idx += num_image

    # np.save('train_warped.npy', train_cropped)
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        # train for one epoch
        train(cfg, train_loader, model, stn, criterion, optimizer, stn_optimizer, epoch,
              final_output_dir, tb_log_dir, writer_dict, cfg.STN.STN_FIRST)
        ...
        msg = 'Outer loop: [{}]\t' \
                  ...
                  'best PCKh@0.5 {}'.format(
                      epoch, round(time.time()-end, 2), perf_indicator, best_perf)
        logger.info(msg)
        ...
        if epoch % 5 == 0:
            save_checkpoint({
                ...
                'stn_state_dict': stn.state_dict(),
                'stn_optimizer': stn_optimizer.state_dict()
            }, False, final_output_dir, filename='stn_{}.pth'.format(epoch+1))
